 ...
 */
export function findExistingStaticFile(sketch, fileName, folder) {
 ...
 */
export function removeFromExistingSketch(existingSketch, staticFile) {
 ...
 */
export function addFileToFilesData(staticFile, parentId, fileUrl, filesData) {
...
export function findStaticFiles(dir, baseDir) {
  const files = [];
...
export function getSketches(folder) {
 ...
 */
export async function createFolderStructure(folder, folderMap, filesData) {
  try {
 ...
 * @returns {Promise<void>}
 */
export async function createFolderStructures(
  staticFiles,
  folderMap,
  filesData
) {
  try {
    ...
    log.error(`Error creating folder structures: ${error}`);
 ...
 */
export const processAllStaticFiles = async (
  staticFiles,
  folderMap,
  filesData,
  existingSketch,
  ...
  log.info(`Processing ${staticFiles.length} static files...`);
  ...
  for (const staticFile of staticFiles) {
    try {
      ...
      log.error(`Error handling static file ${staticFile.name}: ${error}`);
      results.errors++;
    }
  }
 ...
 */
export const processStaticFile = async (
  staticFile,
  folderMap,
  filesData,
  existingSketch,
  ...
  try {
    ...
    if (!fileUrl) {
      log.warning(`No URL available for file ${staticFile.name}, skipping`);
      return null;
    }
    ...
    log.error(`Error processing static file ${staticFile.name}: ${error}`);
    return null;
  }
};
 ...
 * @returns {string} File URL if successful, null otherwise
 */
export const uploadStaticFile = async (
  staticFile,
  existingSketch,
  parentId,
  staticFilesMap
  ...
  try {
    ...
    log.upload(`Uploading ${staticFile.path}...`);
    ...
    if (!uploadResult?.success) {
      log.error(`Failed to upload file ${staticFile.name}`);
    ...
    log.error(`Error uploading static file ${staticFile.name}: ${error}`);
    return null;
  }
};
...
export const processStaticFiles = async (
  ...
  try {
    ...
    log.error(`Error processing static files for sketch ${sketchPath}:`, error);
